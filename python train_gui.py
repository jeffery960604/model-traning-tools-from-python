...
class LoRA_model(torch.nn.Module):
    def __init__(self, original_model, vocab_size=30522):
        ...
        self.lora_input = torch.nn.Linear(original_model.config.hidden_size, 1)
        self.lora_output = torch.nn.Linear(1, vocab_size)
        
    def forward(self, input_ids):
        outputs = self.original_model(input_ids=input_ids)
        hidden_state = outputs.last_hidden_state[:, 0, :]  # 使用CLS标记的隐藏状态
...
class TrainApp:
    ...
    def initialize_model(self):
        # 这里假设你有一个预训练的模型路径，实际使用时需要替换
        model_dir = "pretrained_models"  # 模型存储的目录
        ...
        self.original_model = model  # 将原始模型存储为实例变量
    ...
    def train_model(self):
        ...
        class TextDataset(Dataset):
            ...
            def __getitem__(self, idx):
                ...
                # 这里需要将文本转换为张量（如使用分词器）
                # 例如：假设分词器在 model 的 tokenizer 属性中
                # tokens = model.tokenizer(text1, padding=True, truncation=True, return_tensors='pt')
                # 为简化示例，假设数据已经处理成张量
                return {
                    "input_ids": torch.tensor(text1),  # 假设 text1 已经编码为张量
                    "targets": torch.tensor(text2)      # 假设 text2 是标签
                }
        ...
        os.makedirs(model_dir, exist_ok=True)
        model_path = os.path.join(model_dir, f"{self.model_name}_finetuned.pth")
